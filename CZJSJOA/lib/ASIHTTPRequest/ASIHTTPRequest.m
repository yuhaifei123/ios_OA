...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {
        ...
        if (clientCertificateIdentity) {
            ...
            //升级 ios5 后，证书认证无法使用的问题，原因是 ios5 TLS 版本默认为 1.2 版了
            [sslProperties setObject:(NSString *)kCFStreamSocketSecurityLevelTLSv1 forKey:(NSString *)kCFStreamSSLLevel];
        }
...
- (void)readResponseHeaders
{
	...
	// Handle cookies
	NSArray *tempCookies = [NSHTTPCookie cookiesWithResponseHeaderFields:[self responseHeaders] forURL:[self url]];
	
	NSMutableArray *newCookies = [NSMutableArray arrayWithCapacity:[tempCookies count]];
	
	NSInteger count = [tempCookies count];
	for (NSInteger i = 0; i < count; i++) {
		
		NSHTTPCookie *tempCookie = [tempCookies objectAtIndex:i];
		
		NSString *cookieValue = [[tempCookie value] lowercaseString];
		NSString *cookieName = [[tempCookie name] uppercaseString];
		
		if ([cookieValue rangeOfString:@"deleted"].location == NSNotFound) {//没有过期的
			[newCookies addObject:tempCookie];		
		}
		else {//有过期的
			NSArray *sharedCookies = [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookiesForURL:[[self url] absoluteURL]];
			for (int j = 0; j < [sharedCookies count]; j++) {
				NSHTTPCookie *sharedCookie = [sharedCookies objectAtIndex:j];
				NSString *sharedCookieName = [[sharedCookie name] uppercaseString];
				if ([sharedCookieName isEqualToString:cookieName]) {
					[[NSHTTPCookieStorage sharedHTTPCookieStorage] deleteCookie:sharedCookie];
					break;
				}
			}			
		}
