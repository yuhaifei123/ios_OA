...
@implementation RenWuZhuanXie_TableViewCell
...
- (void)setDeleteDelegate:(id<RenWuZhuanXie_TableViewCellDelegate>)deleteDelegate {
    return objc_setAssociatedObject(self, ASSOCIATION_DELEGATE, deleteDelegate, OBJC_ASSOCIATION_ASSIGN);
}

- (id<RenWuZhuanXie_TableViewCellDelegate>)deleteDelegate {
...
- (void)isEnableDelete {
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(25, 20, 40, 40)];
    label.text = @"删除";
    ...
    deleteImageView.frame = CGRectMake([UIScreen mainScreen].bounds.size.width+20, 0, DELETE_VIEW_WIDTH, self.frame.size.height);
    deleteImageView.userInteractionEnabled = YES;

    [deleteImageView addSubview:label];
...
- (void)leftSwipeAction:(UISwipeGestureRecognizer*)leftSwipe {
    ...
    for (RenWuZhuanXie_TableViewCell* cell in visibleCells) {
