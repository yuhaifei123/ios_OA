...
NSString *vpnIp =    @"58.216.242.87";  //vpn设备IP地址
NSString *userName = @"czscxjsj";             //用户名认证的用户名
NSString *password = @"czscxjsj";                //用户名认证的密码
...
- (void) panDuan{
    ...
    NSString *autoLogin = [as stringForKey:@"autologin"];
...
- (void) ziDongLogin{
    ...
    if([self.name_Text.text isEqualToString:@""]){
        ...
        [IanAlert alertError:@"用户名不能为空" length:abcd];
        ...
        NSTimeInterval abcd = 1.0;
        [IanAlert alertError:@"密码不能为空" length:abcd];
...
//            NSString* aStr;
//            aStr = [[NSString alloc] initWithData:cookiesData encoding:NSUTF8StringEncoding];
