//
//  JJSJJH_XiangQing_ViewController.m
//  CZJSJOA
//
//  Created by 虞海飞 on 16/5/12.
//  Copyright © 2016年 薛伟俊. All rights reserved.
//
#pragma  mark -- 经济审计计划（内审计划）
#import "JJSJJH_XiangQing_ViewController.h"
#import "LPPopupListView.h"
#import "RenWuZhuanXie_SelectMen_TableViewController.h"

@interface JJSJJH_XiangQing_ViewController ()<UITextViewDelegate,LPPopupListViewDelegate,RenWuZhuanXie_SelectMen_TableViewControllerDelegate>
...
/** 计划内容 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_JHNR;
/** 备注 */
...
@implementation JJSJJH_XiangQing_ViewController
...
- (void)viewDidLoad {
    ...
    [self add_GetHttp_PinJie:^NSString *{
        ...
        NSString *string_tableName = @"OA_YWGL_JJSJJH";
        NSString *string_flowName = @"jjsjjh";
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.textView_JHNR.delegate = self;
    ...
    //选人小手判断
    if (![self.dic_AllList[@"JSDE940"] isEqualToString:@"b04"] && [self.dic_AllList[@"STATE"] isEqualToString:@"0"]) {
...
- (void)add_View_Array:(NSArray *)array{
    ...
    self.textView_JHNR.text = dic_list[@"JHNR"];
...
//        NSString *name = [as stringForKey:@"username"];
        ...
        if ([[NSString stringWithFormat:@"%@", self.dic_XiaoShouXuanRen[@"name"]] isEqualToString:@""]) {
          ...
          //  NSLog(@"%@",self.dic_XiaoShouXuanRen);
...
-(void) add_Process{
    ...
    NSString *isxsbm = dic_list[@"ICOUNT"];
    if ([self.liuChenBean.bz isEqualToString:@"b02"] && [isxsbm isEqualToString:@"0"]) {

        self.liuChenBean.spyj = self.textView_ZJJDCYJ.text;
        self.liuChenBean.spyjColumn = @"ZJJDCYJ";
    }
    else if([self.liuChenBean.bz isEqualToString:@"b02"] && [isxsbm isEqualToString:@"1"]){

        self.liuChenBean.spyj = @"";
    ...
    if ([self.liuChenBean.bz isEqualToString:@"b04"] && ![dic_list[@"RYID"] isEqualToString:@""]) {

        self.liuChenBean.idlist = self.dic_ALLXiangQing[@"RYID"];
        self.liuChenBean.names = self.dic_ALLXiangQing[@"RYNAME"];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    NSDictionary *dic = dic = [[NSDictionary alloc] initWithObjectsAndKeys:@"6",@"type",@"jjsjjh",@"rolename",nil];
