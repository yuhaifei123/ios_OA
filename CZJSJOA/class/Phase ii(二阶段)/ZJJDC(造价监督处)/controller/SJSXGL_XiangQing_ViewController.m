//
//  SJSXGL_XiangQing_ViewController.m
//  CZJSJOA
//
...
//
#pragma  mark -- 审计事项管理
#import "SJSXGL_XiangQing_ViewController.h"
#import "LPPopupListView.h"
#import "RenWuZhuanXie_SelectMen_TableViewController.h"

@interface SJSXGL_XiangQing_ViewController ()<UITextViewDelegate,LPPopupListViewDelegate,RenWuZhuanXie_SelectMen_TableViewControllerDelegate>

/** 标题 */
@property (weak, nonatomic) IBOutlet UILabel *label_BT;
...
/** 审计事项 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_SJSX;
/** 备注 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_BZ;
/** 局属单位会签 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_JSDWHQ;
/** 局相关处室会签 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_JXGCSHQ;
/** 分管领导意见 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_FGLDYJ;
/** 主要领导意见 */
@property (weak, nonatomic) IBOutlet Tool_TextView *textView_ZYLD;
...
@implementation SJSXGL_XiangQing_ViewController
...
- (void)viewDidLoad {
    ...
    [self add_GetHttp_PinJie:^NSString *{
        ...
        NSString *string_tableName = @"OA_YWGL_SJSXHQ";
        NSString *string_flowName = @"sjsxhq";
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:
                         self.textView_SJSX,@"SJSX",
                         self.textView_BZ,@"BZ",
                         self.textView_JSDWHQ,@"JSDW",
                         self.textView_FGLDYJ,@"FGLD",
                         self.textView_JXGCSHQ,@"JXGCS",
                         self.textView_ZYLD,@"ZYLDYJ",
                         nil];
    ...
    self.dic_AllLianXi = [NSDictionary dictionaryWithObjectsAndKeys:
                          self.textView_SJSX,@"SJSX",
                          self.textView_BZ,@"BZ",
                          self.textView_JSDWHQ,@"JSDWYJ",
                          self.textView_FGLDYJ,@"FGLDYJ",
                          self.textView_JXGCSHQ,@"JXGCSYJ",
                          self.textView_ZYLD,@"ZYLDYJ",
                          nil];

    self.textView_SJSX.delegate = self;
    self.textView_BZ.delegate = self;
    self.textView_JSDWHQ.delegate = self;
    self.textView_JXGCSHQ.delegate = self;
    self.textView_FGLDYJ.delegate = self;
    self.textView_ZYLD.delegate = self;

    //选人小手判断
    if ([self.dic_AllList[@"JSDE940"] isEqualToString:@"b07"] && [self.dic_AllList[@"STATE"] isEqualToString:@"0"]) {

        self.button_XuanRen.hidden = NO;
    ...
    if ([self.dic_AllList[@"JSDE940"] isEqualToString:@"b03"] && [self.dic_AllList[@"STATE"] isEqualToString:@"0"]) {
    ...
    if ([self.dic_AllList[@"JSDE940"] isEqualToString:@"b05"] && [self.dic_AllList[@"STATE"] isEqualToString:@"0"]) {
...
- (void)add_View_Array:(NSArray *)array{
    ...
    self.label_BT.text = dic_list[@"TITLE"];

    self.textView_SJSX.text = dic_list[@"SJSX"];
    self.textView_BZ.text = dic_list[@"BZ"];
    self.textView_ZYLD.text = dic_list[@"ZYLDYJ"];
    self.textView_JSDWHQ.text = [NSString stringWithFormat:@"%@\n%@",self.textView_JSDWHQ.text,dic_list[@"JSDWYJ"]];
    self.textView_JXGCSHQ.text = [NSString stringWithFormat:@"%@\n%@",self.textView_JXGCSHQ.text,dic_list[@"JXGCSYJ"]];
    self.textView_FGLDYJ.text = [NSString stringWithFormat:@"%@\n%@",self.textView_FGLDYJ.text,dic_list[@"FGLDYJ"]];
    ...
    [self.webView_WD tool_Attachment:string_Attachment_WD];
    ...
    if (self.dic_XiaoShouXuanRen != nil) {
        ...
        if ([[NSString stringWithFormat:@"%@", self.dic_XiaoShouXuanRen[@"name"]] isEqualToString:@""]) {
            ...
            self.textView_FGLDYJ.text = [NSString stringWithFormat:@"并请%@阅。",self.dic_XiaoShouXuanRen[@"name"]];
        }
    }
}
...
-(void) add_Process{
    ...
    if ([self.liuChenBean.bz isEqualToString:@"b03"]) {
        ...
        self.liuChenBean.spyj = [NSString stringWithFormat:@"%@",self.textView_JSDWHQ.text];
        self.liuChenBean.spyjColumn = @"JSDWYJ";
    }
    else if([self.liuChenBean.bz isEqualToString:@"b05"]){

        self.liuChenBean.spyj = [NSString stringWithFormat:@"%@",self.textView_JXGCSHQ.text];
        self.liuChenBean.spyjColumn = @"JXGCSYJ";
    }
    else if([self.liuChenBean.bz isEqualToString:@"b07"]){
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    NSDictionary *dic = dic = [[NSDictionary alloc] initWithObjectsAndKeys:@"6",@"type",@"sjsxhq",@"rolename",nil];
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if (arrray.count < 1) {
        ...
        if ([string_DicKey isEqualToString:@"JSDWYJ"] && [self.dic_AllList[@"JSDE940"] isEqualToString:@"b03"]) {
            string_DicKey = @"JSDW";
        }
        else if ([string_DicKey isEqualToString:@"JXGCSYJ"] && [self.dic_AllList[@"JSDE940"] isEqualToString:@"b05"]){
            string_DicKey = @"JXGCS";
        }
        else if ([string_DicKey isEqualToString:@"FGLDYJ"] && [self.dic_AllList[@"JSDE940"] isEqualToString:@"b07"]){
            string_DicKey = @"FGLD";
        }
